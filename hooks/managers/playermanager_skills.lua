function PlayerManager:_on_enter_trigger_happy_event(unit, attack_data)
	...
	if attacker_unit == self:player_unit() --[[ and variant == "bullet" ]] and not self._coroutine_mgr:is_running("trigger_happy") and
			self:is_current_weapon_of_category("pistol") then
		local data = self:upgrade_value("pistol", "stacking_hit_damage_multiplier", 0)

		if data ~= 0 then
			self._coroutine_mgr:add_coroutine(
					"trigger_happy", PlayerAction.TriggerHappy, self, data.damage_bonus, data.max_stacks, Application:time() + data.max_time
			)
		end
	end
end
...
function PlayerManager:_on_enter_ammo_efficiency_event()
	if not self._coroutine_mgr:is_running("ammo_efficiency") then
		...
		if weapon_unit and weapon_unit:base():fire_mode() ~= "auto" and weapon_unit:base():is_category("smg", "assault_rifle", "snp") then -- oryo: burst marker
			self._coroutine_mgr:add_coroutine(
					"ammo_efficiency", PlayerAction.AmmoEfficiency, self, self._ammo_efficiency.headshots, self._ammo_efficiency.ammo,
